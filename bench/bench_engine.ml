...
let ca, cert =
  let subject = [X509.Distinguished_name.(Relative_distinguished_name.singleton (CN "Miragevpn snakeoil"))] in
  ...
  let extensions =
    ...
    let auth = Some (X509.Public_key.id pubkey), X509.General_name.empty, None in
  ...
  let valid_until = Ptime.add_span valid_from (Ptime.Span.of_int_s 60) |> Option.get in
  let cert =
    match
      X509.Signing_request.sign csr ~valid_from ~valid_until
        ~digest ~extensions
        key subject
    with
    | Ok cert -> cert
    | Error e -> Format.kasprintf failwith "cert error %a" X509.Validation.pp_signature_error e
  in
  cert, cert
...
let [@ocaml.warning "-8"] (initial_client, inital_client_outs, _application, None ) =
  ...
  in
  Engine.handle pre_connect `Connected
  |> Result.get_ok
...
let established_client, establish_server =
  let drain role state inputs =
    let (state, outs) =
      List.fold_left (fun (state, outs) input ->
          match Engine.handle state (`Data input) with
          | Ok (state, outs', _application, None) ->
            Format.eprintf "%s state @[<1>%a@]\n%!" role State.pp state;
            (state, outs' @ outs)
          | Ok (_, _, _, Some act) ->
            Format.kasprintf failwith "Unexpected action %a" State.pp_action act
          | Error e ->
