...
let derive_keys session (key_source : State.key_source) (tls_data : Packet.tls_data) =
  let pre_master, client_random, server_random, client_random', server_random', sids =
    if Cstruct.(equal empty key_source.pre_master) then
      tls_data.pre_master, tls_data.random1, key_source.random1,
      tls_data.random2, key_source.random2,
      (session.their_session_id, session.my_session_id)
    else
      key_source.pre_master, key_source.random1, tls_data.random1,
      key_source.random2, tls_data.random2,
      (session.my_session_id, session.their_session_id)
  in
  let master_key =
    prf ~label:"OpenVPN master secret" ~secret:pre_master
  ...
  | Some data ->
    ...
    let my_key, my_hmac, their_key, their_hmac =
      if Cstruct.(equal empty key.State.pre_master) then
        ...
        Cstruct.sub keys 0 32, Cstruct.sub keys 64 Packet.hmac_len
      else
        Cstruct.sub keys 0 32, Cstruct.sub keys 64 Packet.hmac_len,
        Cstruct.sub keys 128 32, Cstruct.sub keys 192 Packet.hmac_len
    in
...
let kex_server config session (keys : key_source) tls data =
  maybe_kdf ~with_premaster:true session keys data >>= fun (_tls_data, keys_ctx) ->
  ...
  let td = { Packet.pre_master = Cstruct.empty ; random1 = keys.random1 ; random2 = keys.random2 ; options ; user_pass = None } in
  match Tls.Engine.send_application_data tls [ Packet.encode_tls_data td ] with
...
let push_reply tls data =
  ...
  let repl = Cstruct.concat [ Packet.push_reply ; Cstruct.of_string data ; Cstruct.create 1 ] in
...
let incoming_control_server is_not_taken config rng session channel _now _ts _key op data =
  match channel.channel_st, op with
  | Expect_reset, (Packet.Hard_reset_client | Packet.Soft_reset) ->
    ...
    in
    let tls_config = Tls.Config.server ~certificates:(`Single ([server], key)) () in
    let tls = Tls.Engine.server tls_config in
    let channel = { channel with channel_st = TLS_handshake tls } in
    let control_typ = match Config.find Ifconfig config with
      | None -> `Reset_server
      | Some _ -> `Reset
    in
      ...
      | Some data ->
        if Cstruct.(equal Packet.push_request data) then
          ...
          let ping = match Config.get Ping_interval config with
            | `Not_configured -> 10
            | `Seconds n -> n
          and restart = match Config.get Ping_timeout config with
...
let incoming_control is_not_taken config rng state session channel now ts key op data =
  ...
  match state with
  | Client _ ->
    incoming_control_client config rng session channel now op data >>| fun (est, config', ch'', outs) ->
...
let wrap_hmac_control now ts mtu session key transport outs =
  ...
  let session, transport, outs =
    List.fold_left (fun (session, transport, acc) (typ, out) ->
        ...
        let session, transport, p =
          ...
          | `Reset_server ->
            let session, transport, header = header session transport now_ts in
            let transport, m_id = next_message_id transport in
            session, transport, [ `Control (Packet.Hard_reset_server, (header, m_id, out)) ]
          | `Reset ->
            let session, transport, header = header session transport now_ts in
            let transport, m_id = next_message_id transport in
            session, transport, [ `Control (Packet.Soft_reset, (header, m_id, out)) ]
        in
        (* hmac each outgoing frame and encode *)
        let out = List.map (hmac_and_out session.protocol key session.my_hmac) p in
        let out_packets =
          List.fold_left2 (fun m pkt out -> match pkt with
              | `Ack _ -> m
...
let merge_payload a b = match a, b with
  | None, None -> None
  | Some a, None -> Some a
         ...
         | `Data data ->
           begin match keys_opt ch with
             ...
             | Some keys ->
               ...
               incoming_data bad_mac keys state.session.compress data >>| fun payload ->
         ...
         | (`Ack _ | `Control _) as d ->
           ...
           | Ok (session, transport) ->
             ...
             | `Control (typ, (_, _, data)) ->
               ...
               let outs = match outs with [] -> [ `Ack, Cstruct.empty ] | xs -> xs in
               ...
               let out = out @ encs
               and ch = { ch with transport }
               and state = { state with config ; session }
               in
               match est with
