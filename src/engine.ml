...
let incoming_control_client config rng session channel now op data =
  ...
  | Expect_reset, (Packet.Hard_reset_server_v2 | Packet.Soft_reset_v2) ->
      ...
      let tls, ch =
        let authenticator =
          match Config.find Ca config, Config.find Peer_fingerprint config with
          | None, None ->
              Log.warn (fun m ->
                  m "not authenticating certificate (missing CA and peer-fingerprint)");
          ...
          | _, Some fps ->
              Logs.info (fun m ->
                m "authenticating with fingerprints %a"
                  Fmt.(list ~sep:(any "\n") Hex.pp) (List.map Hex.of_cstruct fps));
              fun ?ip ~host chain ->
                List.fold_left
                  (fun acc fingerprint ->
                     match acc with
