...
let server ?(really_no_authentication = false) ~is_not_taken ?auth_user_pass
    server_config =
  let open Result.Syntax in
...
let derive_keys ~tls_ekm ~peer_id session (my_key_material : State.my_key_material)
  ...
  let keys, peer_id =
    ...
    | Some tls ->
        ...
        Tls.Engine.export_key_material epoch "EXPORTER-OpenVPN-p2p-peerid" 3
  in
  (server, keys, peer_id)
...
let maybe_kex_client config tls =
  ...
  if Tls.Engine.handshake_in_progress tls then Ok (TLS_handshake tls, None)
    ...
    let pre_master, random1, random2 =
      ( Mirage_crypto_rng.generate 48,
...
let kdf ~tls_ekm ~peer_id session cipher hmac_algorithm my_key_material
  ...
  in
  { my_replay_id = 1l; their_replay_id = 1l; keys; peer_id }
    ...
    let+ state =
      ...
      | None ->
          ...
          if requested_push then
            ...
            Ok
              (`State
                 ( Push_request_sent (tls', my_key_material, their_tls_data),
                   None ))
...
let incoming_control_server auth_user_pass is_not_taken config session channel
    _key op data =
  let open Result.Syntax in
  match (channel.channel_st, op) with
  | ( Expect_reset,
  ...
  | TLS_handshake tls, Packet.Control -> (
      ...
      let channel_st =
        if Tls.Engine.handshake_in_progress tls' then TLS_handshake tls'
          ...
          let random1, random2 =
            (Mirage_crypto_rng.generate 32, Mirage_crypto_rng.generate 32)
          and pre_master = "" in
    ...
    Option.fold ~none:(Ok ())
      ~some:(fun seq ->
        guard
          ...
          (`Non_monotonic_sequence_number (transport.their_sequence_number, seq)))
      sn
  in
  let session =
    {
  ...
  *)
  ...
  let aead (type key) tag_size
    ...
    let data =
      ...
      if compress then "\xfa" ^ data else data
    in
    let b =
      Bytes.create
...
let data_out ?add_timestamp (ctx : keys) hmac_algorithm compress protocol key
  ...
  (* TODO: decide whether to use peer id or not! *)
  let peer_id = Some ctx.peer_id in
(*    if ctx.peer_id = "\xff\xff\xff" then None else Some ctx.peer_id
      in *)
  let adata_pre =
    match peer_id with
    | None -> ""
  ...
  in
  Packet.encode_data out peer_id protocol key;
...
let outgoing s data =
  ...
  | `Static keys, _ ->
      ...
      Ok
        ( {
            ...
            control_crypto;
            channel;
            last_sent = Mirage_mtime.elapsed_ns ();
          },
          out )
  ...
  | _, Some ctx ->
      ...
      let hmac_algorithm = Config.get Auth s.config in
      let ctx, out =
        data_out ctx hmac_algorithm sess.compress sess.protocol s.channel.keyid
          data
      in
...
let maybe_rekey state =
  ...
  let should_rekey =
    ...
    | Some y, _, _
      when y
           <= Duration.to_sec
                (Int64.sub (Mirage_mtime.elapsed_ns ()) state.channel.started)
           && y > 0 ->
        true
...
let incoming_data ?key_op ?(add_timestamp = false) err (ctx : keys) hmac_algorithm
  ...
  let* data =
    let split_aead tag_len =
      ...
      let* () =
        guard
          ...
          (`Payload_too_short (prefix_len, String.length data))
      in
    ...
    | AES_GCM { their_key; their_implicit_iv; _ } ->
        ...
        let* _peer_id, replay_id, adata, tag_off, off = split_aead tag_len in
        ...
        let valid =
          Mirage_crypto.AES.GCM.authenticate_decrypt_into ~key:their_key ~nonce
            ~adata data ~src_off:off ~tag_off plain ~dst_off:0
...
let find_channel state key op =
  ...
  | None -> (
      ...
      | ( ( Client
          ...
          Packet.(
            ( Control | Ack | Data_v1 | Data_v2 | Hard_reset_client_v2
...
let received_data ?key_op state ch set_ch payload =
  let open Result.Syntax in
  ...
  | None ->
      ...
      Ok (state, None)
  | Some keys ->
      Log.debug (fun m -> m "received data!");
              ...
              | `Control (_, (_, _, data)) -> (
                  let* est, config, ch, out' =
                    incoming_control state.auth_user_pass state.is_not_taken
                      state.config state.state state.session ch key op data
                  in
                  Log.debug (fun m ->
