module Config : sig
  ...
  type 'a k =
    ...
    | Auth_retry : [ `Interact | `Nointeract | `None ] k
        (** [`Interact]: Interactively ask user for new Auth_user_pass value
            before retrying an authentication attempt. [`Nointeract]: Retry an
            authentication attempt with same Auth_user_pass [`None]: Exit with
            fatal error if authentication fails (default) *)
    ...
    | Connect_retry_max : [ `Unlimited | `Times of int ] k
        (** How many times to retry each remote before giving up. The counter
            for each remote/connection should be reset upon the successful
            establishment of a connection.*)
    | Connect_timeout : int k
        (** [connect-timeout] a.k.a. [server-poll-timeout] Timeout a connection
            attempt to a remote server after [SECONDS]. *)
    | Data_ciphers : [ `AES_128_GCM | `AES_256_GCM | `CHACHA20_POLY1305 ] list k
    | Dev : ([ `Tun | `Tap ] * string option) k
        (** The device to be used for the unencrypted traffic. The optional
            string is a device name, the [dev] directive. The polymorphic
            variant specifies the [dev-type]. The [dev-type] is inferred from
            the [dev] name if the latter consists of a "tap" or "tun" prefix
            followed by an integer between 0 and 128. If no device name is
            given, or the device name is simply "tun" or "tap", the device must
    ...
    | Handshake_window : int k
        (** TLS-based key exchange must finalize within [n] seconds of handshake
            ...
            our connection with the peer and try again. Even in the event of
            ...
            tunnel data. TODO pasted from `man openvpn` *)
    ...
    | Ping_interval : [ `Not_configured | `Seconds of int ] k
        (** The [ping] config directive. Will send a OpenVPN ping packet (ie
            {b not} IP/ICMP) over the control channel after [seconds] of
            inactivity on the control/data channel to tell the peer that we are
            still alive. Defaults to [`Not_configured] (equivalent to `ping 0`),
            which means no pings will be sent. TODO ?The peer will be notified
            of the value of this configuration upon connection, and thus will
            know whether or not to expect pings from us? *)
    ...
    | Tls_mode : [ `Client | `Server ] k
        (** Governed by the [tls-client] and [tls-server] directives. Indirectly
            also by [client]. *)
    | Tls_key : X509.Private_key.t k
    | Tls_timeout : int k
        (** Retransmit control channel packet after not receiving an ACK for [n]
            seconds. TODO: presumably does not apply to TCP? *)
    | Tls_version_min : (Tls.Core.tls_version * bool) k
        (** [v * or_highest]: if [or_highest] then v = the highest version
            supported by the TLS library. *)
  ...
  (** Apply config excerpt from server received upon initial connection.
      ...
      [server_config] does not conflict with [client_config]. TODO return
      conflicting subset as error
      - atm: do what validate_server_options did, returning unmodified. *)

  val merge_push_reply : t -> string -> (t, [> `Msg of string ]) result
  (** [merge_push_reply client_config push_config] is a successful merge of
      [client_config] and [push_config] if [push_config] does not conflict with
      [client_config]. TODO return conflicting subset as error *)

  val parse_server :
    string_of_file:(string -> (string, [ `Msg of string ]) result) ->
    string ->
    (t, [> `Msg of string ]) result
  (** Parses a configuration string, looking up references to external files as
      needed. Validates the server configuration. Default options are applied.
  *)
  ...
  val parse_client :
    ...
    (t, [> `Msg of string ]) result
  (** Parses a configuration string, looking up references to external files as
      needed. Validates the client configuration. Default options are applied.
  *)
end
...
(** [client config] constructs a [t], returns the remote to connect to, an
    initial buffer to send to the remote. It returns an error if the
    configuration does not contain a tls-auth element. *)
...
val server :
  ...
  ( server * (Ipaddr.V4.t * Ipaddr.V4.Prefix.t) * int,
    [> `Msg of string ] )
  result
(** [server config ~is_not_taken ~auth_user_pass] constructs a [server], its
    [ip, netmask] and [port]. The callback [is_not_taken] is provided to avoid
    IP address collisions. The callback [auth_user_pass] validates username and
    password of each connection. It returns an error if the configuration does
    not contain a tls-auth element. *)
...
(** [outgoing t data] prepares [data] to be sent over the OpenVPN connection. If
    the connection is not ready yet, [`Not_ready] is returned instead. *)
...
(** [new_connection server] is to be called when the server accepted a new TCP
    connection, a state [t] is constructed - which can be used with {!handle}.
*)
