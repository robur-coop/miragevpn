...
let hex_of_lines ~name seq =
  ...
  | header :: hex_and_footer when header = "-----BEGIN " ^ name ^ "-----" -> (
      ...
      if footer <> "-----END " ^ name ^ "-----" then
        ...
        try Ok (Ohex.decode (String.concat "" hex))
  ...
  let of_cstruct cs =
    ...
    | 0 ->
        if String.length cs > 1 then
          Ok (User (String.sub cs 1 (String.length cs - 1)))
...
module Tls_crypt : sig
  ...
  let of_cstruct buf =
    ...
    in
    let* server_key = Key.of_cstruct (String.sub buf 0 128) in
    let* client_key = Key.of_cstruct (String.sub buf 128 128) in
  ...
  let load_v1 lines =
    ...
    in
    let* a = Key.of_cstruct (String.sub str 0 128) in
    let* b = Key.of_cstruct (String.sub str 128 128) in
    Ok (a, b)
  ...
  let save_v1 (a, b) =
    let k = Key.unsafe_to_cstruct a ^ Key.unsafe_to_cstruct b in
...
module Wrapped_key : sig
  ...
  let of_cstruct buf =
    ...
    (String.sub buf 0 mid, String.sub buf mid net_len)
  ...
  let wrap ~key:server_key key metadata =
    ...
    let a_b_meta = String.concat "" [ a; b; metadata ] in
  ...
  let unwrap ~key:server_key wkc =
    ...
    in
    let* key = Tls_crypt.of_cstruct (String.sub key_and_metadata 0 256) in
    let metadata =
