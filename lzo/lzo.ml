...
module Logs = (val Logs.(src_log @@ Src.create "ovpn.lzo") : Logs.LOG)
...
type instruction =
  | Copy_literal of int * copy_trailing
  (* literal run from input *)
  (* Invariants:
  ...
  *)
  | Copy_block of int * int * copy_trailing
  (* looped run from dictionary *)
  (* Invariants:
  ...
  *)
  | Read_byte of (int -> (instruction, string) result)
  (* read extra operand *)
  (* Invariants:
...
let[@inline always] count_zeroes f =
  ...
  *)
  let rec do_count count =
    if count >= 4210750 then
      Error "LZO: count_zeroes literal length would overflow"
    else
      Ok
        (Read_byte
           (function
...
let decode_instruction (prev_state : copy_trailing) c =
  ...
  | '\016' .. '\031', _ (* x10 - 1F *) ->
        ...
        Read_le16
          (fun ds ->
            ...
            let distance =
              ...
              16384 + (h lsl 14) + (ds lsr 2)
            in
            (* each of these add up to 16kB*)
            match distance with
            | 16384 -> End_of_stream (* still need to copy TODO what? *)
      ...
      if l = 0 then count_zeroes (fun l -> with_length (2 + 7 + l))
  ...
  | '\032' .. '\063', _ (* x20 - 3F *) ->
      ...
      (* & 31 *)
      if l = 0 then count_zeroes (fun l -> with_length (2 + 31 + l))
      else Ok (with_length (2 + l))
  | '\064' .. '\255', _ (* x40 - FF *) ->
      ...
      let length, d =
        ...
        (* 3 + shift + ((b >> 5) & (shift+1)): *)
        (3 + shift + ((b lsr 5) land (1 lor shift)), (b lsr 2) land 0b111)
      in
      (*      (b >> 2) & 7  *)
      Ok
        (Read_byte
           (fun h ->
             ...
             Ok (Copy_block (length, distance, copy_trailing_of_int b))))

let decompress (input : string) =
  let out = Buffer.create 1500 in
  Logs.debug (fun m ->
      m "LZO input is %d bytes: %S" (String.length input) input);
  let open Rresult in
  ((* Fail "gracefully" with large inputs on 32-bit platforms
   ...
   if potential < Int64.(of_int Sys.max_string_length) then Ok ()
   else
     R.error_msgf
       "LZO: You're using this module on a platform that will throw exceptions \
        when trying to allocate strings larger than %d. This input could \
        potentially expand to %Ld cause an exception when the Buffer module \
        fails to allocate a large string."
       Sys.max_string_length potential)
  >>= fun () ->
  let rec handle_instruction inp = function
    ...
    | _ when inp >= String.length input -> Error "Would read past input"
    | Copy_literal (len, _) when inp + len >= String.length input ->
        Error "Copy_literal would read past input bounds"
    | Copy_literal (len, state) ->
        (if String.length input - inp < len then
           Error "Copy_literal would read past end of input"
         else Ok (Buffer.add_substring out input inp len))
        >>= fun () ->
        let inp = inp + len in
    ...
    | Copy_block (length, distance, state) ->
        ...
        let rec loop off rem =
          ...
          if taken < 0 || rem < 0 || (taken = 0 && rem <> 0) then
            Error "LZO: TODO use math or whatever to prove this can't happen"
          else if taken = 0 then Ok ()
          else (
            Buffer.add_string out (Buffer.sub out off taken);
            loop (off + taken) (rem - taken))
        in
        loop (Buffer.length out - distance) length >>= fun () ->
