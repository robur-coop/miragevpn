...
module Server (R : Mirage_random.S) (M : Mirage_clock.MCLOCK) (P : Mirage_clock.PCLOCK) (T : Mirage_time.S) (S : Tcpip.Stack.V4V6) = struct
  ...
  let callback t flow =
    ...
    let rec read ?ip f =
      ...
      | Ok `Data cs ->
        match
          Miragevpn.handle !client_state ~is_not_taken (`Data cs)
        with
        | Error msg ->
          Log.err (fun m -> m "%a internal miragevpn error %a"
                      pp_dst dst Miragevpn.pp_error msg);
          rm ();
          Lwt.return_unit
        | Ok (s', out, action) ->
          client_state := s';
          let ip =
            match action with
            | Some `Established  ({ Miragevpn.cidr ; _ }, _) ->
            ...
            | Some a ->
              Log.warn (fun m -> m "%a ignoring action %a"
                           pp_dst dst Miragevpn.pp_action a);
              `Continue ip
            | None -> `Continue ip
          in
  ...
  let connect config stack =
    ...
    | Error `Msg msg ->
      ...
      assert false
    | Ok (server, ip, port) ->
      Log.info (fun m -> m "miragevpn server listening on port %d, using %a/%d"
...
module Make (R : Mirage_random.S) (M : Mirage_clock.MCLOCK) (P : Mirage_clock.PCLOCK) (T : Mirage_time.S) (S : Tcpip.Stack.V4V6) = struct
  ...
  let rec event s conn =
    ...
    Log.debug (fun m -> m "now for real processing event %a" Miragevpn.pp_event ev);
    ...
    | Ok (o_client, action) ->
      ...
      let rec established () =
        ...
        Log.debug (fun m -> m "tunnel re-established (ip changed? %B) %a (mtu %d)"
                      ip_changed Miragevpn.pp_ip_config ip_config' mtu');
        if ip_changed then
